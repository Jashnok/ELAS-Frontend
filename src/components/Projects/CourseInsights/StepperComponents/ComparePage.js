import { Badge, Button, Card, CardContent, createMuiTheme, Tab, Tabs, Tooltip, Typography, ThemeProvider } from "@material-ui/core";
  ...
  return (
    <React.Fragment>
      <TableRow className={classes.root}>
        ...
        <TableCell align="center">{row.fun ? Math.round(row.fun) : <CloseIcon className={classes.closeicons} />}</TableCell>
        <TableCell align="center">{row.understandability ? Math.round(row.understandability) : <CloseIcon className={classes.closeicons} />}</TableCell>
        ...
        <TableCell align="left" title={"Delete"}>
          ...
          {/*<DeleteIcon onClick={() => props.deleteRow(row)} style={{cursor: "click"}}/>*/}
        </TableCell>
      </TableRow>
      <TableRow>
  ...
  return (
    <React.Fragment>
      <TableRow className={classes.root}>
        ...
        <TableCell align="center">{row.fairness ? Math.round(row.fairness) : <CloseIcon className={classes.closeicons} />}</TableCell>
        <TableCell align="center">{row.support ? Math.round(row.support) : <CloseIcon className={classes.closeicons} />}</TableCell>
        <TableCell align="center">{row.material ? Math.round(row.material) : <CloseIcon className={classes.closeicons} />}</TableCell>
        <TableCell align="center">{row.fun ? Math.round(row.fun) : <CloseIcon className={classes.closeicons} />}</TableCell>
        <TableCell align="center">{row.understandability ? Math.round(row.understandability) : <CloseIcon className={classes.closeicons} />}</TableCell>
        <TableCell align="center">{row.node_effort ? Math.round(row.node_effort) : <CloseIcon className={classes.closeicons} />}</TableCell>
        ...
        <Grid item style={{ marginTop: 25, alignSelf: "center", width: "100%" }}>
            ...
            <TabContext value={value}>
              <ThemeProvider theme={theme}>
                <Tabs value={value} onChange={handleChange} indicatorColor="primary">
                  ...
                  {removedSubjects.length === 0 ? <Tab label="removed subjects" disabled value="2" /> : <Tab label="removed subjects" value="2" style={{ color: "#f50057" }} />}
                </Tabs>
              </ThemeProvider>
              ...
              <TabPanel value="2">
                  ...
                  <Table stickyHeader aria-label="collapsible table" cellSpacing="1">
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell align="center" style={{ width: 15 }} title={"Understandability"}>
                          Underst.
                        </TableCell>
                        ...
                        <TableCell align="center" style={{ width: 15 }} title={"More information"}>
                          Info
                        </TableCell>
                ...
                <TablePagination rowsPerPageOptions={[5, 10, 20, 30]} component="div" count={removedSubjects.length} rowsPerPage={rowsPerPage} page={page} onChangePage={handleChangePage} onChangeRowsPerPage={handleChangeRowsPerPage} />
              </TabPanel>
            </TabContext>
          </Grid>
          <Grid item style={{ width: "100%", marginTop: 20 }}>
            <Grid container spacing={2}>
              <Grid item>
                <Button variant="outlined" color="primary" onClick={props.handleBack()} className={classes.button}>
                  Back
                </Button>
              </Grid>
              <Grid item>
                <Button variant="contained" color="secondary" onClick={props.handleReset()} className={classes.button}>
                  Reset
                </Button>
              </Grid>
            </Grid>
